...
class AdminCustomers extends Admin {
    ...
    public function index($field='firstname', $by='ASC', $page=0)
    {
    ...
    function contact_list(){
        $data['contact_list'] = \CI::db()->order_by('id', 'DESC')->limit(100)->get('contacts')->result();
        $this->view('contact_list', $data);
    }
    ...
    function suppliers(){
        if(isset($_POST['submit'])){
            ...
            foreach($name as $key=>$value){
                $row = array(
                    'name'      => trim($name[$key]),
                    'address'   => trim($address[$key]),
                    'phone'     => trim($phone[$key]),
                    'email'     => trim($email[$key])
                );
                if($key == 0 && $value != ''){
                    // insert new
                    $check_name = \CI::db()->where('name', trim($name[$key]))->get('suppliers')->row();
                    if(!$check_name){
                        \CI::db()->insert('suppliers', $row);
                    }
                }
                if($key > 0) {
                    // update
                    \CI::db()->where('id', $key)->update('suppliers', $row);
                }
            }
        }
        $data['suppliers'] = \CI::db()->where('status', 1)->order_by('id', 'DESC')->get('suppliers')->result();
        $this->view('supplier_list', $data);
    }
