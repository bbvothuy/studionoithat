...
<?php if($requiresShipping):?>
    ...
    foreach ($rates as $key => $rate):
        ...
        <tr onclick="$(this).find('input').prop('checked', true).trigger('change');">
            <td style="width:20px;"><input type="checkbox" name="shippingMethod" value="<?php echo $hash;?>" <?php echo (is_object($selectedShippingMethod) && $hash == $selectedShippingMethod->description)?'checked':'';?>></td>
            <td><?php echo $key;?></td>
