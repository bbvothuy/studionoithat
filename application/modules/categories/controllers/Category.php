    ...
    public function index($slug, $page=0) {
		...
		*/
		...
		if($categories['category']->id == 16){
			...
			//echo $categories['category']->name;exit;
			$categories['keyword'] 	 = $categories['category']->name . ',' . str_replace(" ",',', $categories['category']->name);
			if($categories['category']->keyword != ''){
				$categories['keyword'] = $categories['keyword'] . ','. $categories['category']->keyword;
			}
			$categories['meta'] = $categories['category']->meta ? $categories['category']->meta : $categories['category']->name;
