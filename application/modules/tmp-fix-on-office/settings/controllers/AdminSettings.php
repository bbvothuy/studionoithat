...
class AdminSettings extends Admin {
	...
	public function form_store($store_id = 0){
        if(config_item('store_name') != 'default') {
            $store = config_item('store');
            if($store_id == 0 || $store_id != $store->id)
                redirect('admin');
        }
		\CI::load()->helper('form');
        \CI::load()->library('form_validation');
		...
		if($store_id > 0){
			$store = \CI::Settings()->get_stores($store_id);
			$data = [
				'store_name' => $store->store_name,
				'theme' => $store->theme,
				'domain' => $store->domain,
				'default_meta_keywords' => $store->default_meta_keywords,
				'default_meta_description' => $store->default_meta_description,
				'config_price' => $store->config_price,
				'email_from' => $store->email_from,
				'email_to' => $store->email_to,
				'store_id' => $store_id,
                'style' => $store->style,
			];
		}
		...
		if(!empty($_POST)){
			\CI::form_validation()->set_rules('store_name', 'lang:store_name', 'required');        
			\CI::form_validation()->set_rules('theme', 'lang:theme', 'required');
			\CI::form_validation()->set_rules('email_from', 'lang:email_from', 'required|valid_email');
			\CI::form_validation()->set_rules('email_to', 'lang:email_to', 'required|valid_email');
			if (\CI::form_validation()->run() == FALSE)
			{
				$data['error'] = validation_errors();echo 'aaaaa';
				$this->view('form_store', $data);
			}
			else
			{
				\CI::session()->set_flashdata('message', lang('config_updated_message'));
				...
				if($store_id == 0){
					\CI::Settings()->save_stores($save);
				}else{
					\CI::Settings()->update_stores($save, $store_id);
				}
	...
	public function delete_store($id)
    {
        ...
        \CI::Settings()->delete_store($id);
        \CI::session()->set_flashdata('message', lang('message_deleted_message'));
        redirect('admin/settings/stores');
    }
	
	
	public function stores($store_id = 0){
        if(config_item('store_name') != 'default') {
            redirect('admin');
        }
		$data['stores'] = \CI::Settings()->get_stores();
		$this->view('stores', $data);
	}
