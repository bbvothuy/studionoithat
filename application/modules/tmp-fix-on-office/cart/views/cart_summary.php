<div class="cartSummary">
        ...
        <div class="cartItem" id="cartItem-<?php echo $product->id;?>">
            <div class="col-nest">
                <div class="col" data-cols="1">
                    <div class="cartItemName"><a href="<?php echo site_url('product/'.$product->slug);?>"><?php echo $product->name; ?></a></div>
                </div>
            </div>
    ...
    <?php if(count($charges['products']) > 0):?>
        ...
        <div class="cartSummaryTotals">
			...
			<?php $total_price = 0; if(isset($charges['shipping']))foreach($charges['shipping'] as $shipping): $total_price = $total_price + $shipping->total_price; endforeach;?>
			<div class="col-nest">
				<div class="col" data-cols="2/3" data-medium-cols="2/3" data-small-cols="2/3">
					<div class="cartSummaryTotalsKey"><?php echo lang('shipping');?>: <?php echo isset($shipping->name) ? $shipping->name : ''; ?></div>
				</div>
				<div class="col" data-cols="1/3" data-medium-cols="1/3" data-small-cols="1/3">
					<div class="cartSummaryTotalsValue total-shipping">
						<?php echo format_currency($total_price); ?>
					</div>
				</div>
			</div>
                
		
		<?php if(count($charges['tax']) > 0 ):?>
        ...
        <?php endif;?>
            ...
            <div class="col-nest">
                ...
                <div class="col" data-cols="1/3" data-medium-cols="1/3" data-small-cols="1/3">
                    <div class="cartSummaryTotalsValue total-price"><?php echo format_currency(GC::getGrandTotal());?></div>
                </div>
            </div>
        </div>
    <?php endif;?>
    ...
    <div style="padding-bottom:0px; padding-top:10px;border-bottom: 1px solid #e0e0e0;border-top: 1px solid #e0e0e0;">

        <div class="col-nest">
            <div class="col" data-cols="1/4">Ghi chú</div>
            <div class="col" data-cols="3/4">
                <textarea id="notes_customer" name="notes_customer" class="form-control" cols="" rows=""><?php echo GC::getCart()->notes_customer;?></textarea>
            </div>
        </div>

    </div>

	<?php $checkOrder = GC::checkOrder(); if(empty($checkOrder)){?>
		<div class="button-product-detail" style="text-align: right; padding:20px;">
			<a href="/" class="btn btn-cart" id="btn_cod"> Tiếp tục mua hàng </a>
			<button class="btn btn-cart btn-booking-order" id="btn_cod"> Đặt mua hàng </button>
            <!--<button class="btn btn-cart btn-booking-order" id="btn_cod" onclick="CodSubmitOrder()"> Đặt mua hàng </button>-->
		</div>
	<?php }?>
	
	<div style="text-align:right; margin-bottom:10px"><i>(Giá bán tại kho Tp.HCM, chưa bao gồm VAT)</i></div>
</div>
...
function CodSubmitOrder()
{
    ...
    $.post('<?php echo base_url('/cod/process-payment');?>', {'notes_customer': $("#notes_customer").val()}, function(data){
    ...
    $( ".btn-booking-order" ).click(function(){
		...
		if(firstname == '' ||  email == '' || phone == '' || address1 == ''){
			if(firstname == ''){
				$("#firstname").addClass('info-required');
			}
			if(email == ''){
				$("#email").addClass('info-required');
			}
			if(phone == ''){
				$("#phone").addClass('info-required');
			}
			if(address1 == ''){
				$("#address1").addClass('info-required');
			}
			bootbox.alert('Quý khách vui lòng nhập đầy đủ thông tin để thuận tiện liên hệ. <br><span class="red">(*)</span> là thông tin yêu cầu cần có.');
			return false;
		}
        ...
        $.LoadingOverlay('show');
        submit_order = true;
        $.post($('#addressForm').attr('action'), $('#addressForm').serialize(), function(data){
            if(data == 1)
            {
                closeAddressForm();
				//CodSubmitOrder();
