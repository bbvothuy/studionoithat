<div class="page-header">
    <h1><?php echo lang('order');?>: <?php echo $order->id;?></h1>
</div>
<div class="row">
    <div class="col-md-6">
        ...
        <?php foreach($charges['products'] as $product):?>
            <tr>
                <td>
                    ...
                    <a href="<?php echo '/product/'.$product->slug;?>" target="_blank"><?php echo (!empty($product->sku))?'<small>'.lang('sku').': '.$product->sku.'</small>':''?></a>
						...
						if($product->product_type == 2 && $product->combo_product != ''){
							...
							if(is_array($combo)){?>
								<?php foreach($combo as $combo_detail){
									if($str_combo_name == ''){
										$str_combo_name = '<div>Bộ combo gồm: '.'<a href="/product/'.$combo_detail->slug.'" target="_blank">'.$combo_detail->name.'</a>';
									}
									else{
