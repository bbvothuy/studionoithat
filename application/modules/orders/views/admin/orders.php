...
<div class="row">
        ...
        <?php echo form_open('admin/orders', 'class="form-inline" style="float:right"');?>
			<div class="form-group">
				...
				<select name="status" class="form-control">
					...
					<option value="<?php echo $os;?>" <?php if(isset($term->status) && $term->status == $os) echo 'selected="selected"';?>><?php echo $os;?></option>
					<?php endforeach;?>
				</select>
            </div>
			
			<div class="form-group">
                <label class="sr-only" for="search_delivery_date">Delivery Date</label>
                <input name="search_delivery_date" class="datepicker form-control" type="text" placeholder="Delivery Date" value="<?php if(isset($term->search_delivery_date) && $term->search_delivery_date != '') echo date('d/m/Y', strtotime($term->search_delivery_date));?>"/>
            </div>
            <div class="form-group">
                <label class="sr-only" for="start_date"><?php echo lang('start_date');?></label>
                <input name="start_date" value="<?php if(isset($term->start_date) && $term->start_date != '') echo date('d/m/Y', strtotime($term->start_date));?>" class="datepicker form-control" type="text" placeholder="<?php echo lang('start_date');?>"/>
            </div>
            <div class="form-group">
                <input name="end_date" value="<?php if(isset($term->end_date) && $term->end_date != '') echo date('d/m/Y', strtotime($term->end_date));?>" class="datepicker form-control" type="text"  placeholder="<?php echo lang('end_date');?>"/>
            </div>
            <div class="form-group">
                <input style="width: 100%" id="top" type="text" class="form-control" name="term" value="<?php if(isset($term->term)) echo $term->term;?>" placeholder="<?php echo lang('term')?>" />
            </div>
	...
	<div class ="pull-right" style="margin-right:65px"> <sub>Search Order ID, Name, Phone, Email, Notes...</sub></div>
</div>
...
<table class="table table-striped">
    ...
    <tr>
        <td style="white-space:nowrap">
            <strong><a href="<?php echo site_url('admin/orders/order/'.$order->order_number);?>">Order ID: <?php echo $order->invoice_number; ?></a></strong>
            <div style="font-size:11px;"><?php echo date('d/m/Y h:i a', strtotime($order->ordered_on)); ?></div>
        </td>
        <td><?php if($order->store_name == '') echo "Main"; else echo $order->store_name;?></td>
        <!--<td style="white-space:nowrap">
