...
Class Settings
{
    ...
    */
	...
	public function save_stores($values)
    {
        CI::db()->insert('stores', $values);
    }
	
	public function update_stores($values, $store_id)
    {
		CI::db()->where('id', $store_id);
        CI::db()->update('stores', $values);
    }
	
	public function delete_store($store_id)
    {
        CI::db()->where('id', $store_id);
        CI::db()->update('stores', array('status' => 0));
    }
	
    public function get_stores($store_id = 0, $status = '')
    {
		$result = null;
		if($status !== ''){
		...
		if($store_id !== 0){
			CI::db()->where('(id = '.CI::db()->escape($store_id) .' OR domain = '.CI::db()->escape($store_id).')');
		}
