...
class Locations extends CI_Model 
{
    ...
    public function get_city_zones_menu($country_zone_id = 0)
    {
		if($country_zone_id > 0){
			$zones  = CI::db()->select('zone.*, zone_id as id')->where(array('country_zone_id'=>$country_zone_id))->get('zone')->result();
		}else{
			$zones  = CI::db()->select('zone.*, zone_id as id')->get('zone')->result();
		}
        $return = [];
        foreach($zones as $z)
        {
	...
	public function get_ward_menu($zone_id = 0)
    {
		if($zone_id > 0){
			$zones  = CI::db()->select('ward.*, ward_id as id')->where(array('zone_id'=>$zone_id))->get('ward')->result();
		}else{
			$zones  = CI::db()->select('ward.*, ward_id as id')->get('ward')->result();
		}
        $return = [];
        foreach($zones as $z)
        {
