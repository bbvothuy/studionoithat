...
class AdminProducts extends Admin {
    ...
    public function __construct()
    {
        ...
        $this->admin = \CI::session()->userdata('admin');
    ...
    public function index($rows=100, $order_by="id", $sort_order="DESC", $code=0, $page=0)
    {
        ...
        if($this->admin['group_id'] == 5){ // supplier
            $this->view('supplier_products', $data);
        }else {
    ...
    public function form($id = false, $duplicate = false)
    {
        ...
        $data['suppliers'] = \CI::db()->get('suppliers')->result();
        ...
        if($this->admin['group_id'] == 5){
            $data['confirm'] = 0;
            $data['supplier_id'] = $this->admin['supplier_id'];
        }
        ...
        */
        ...
        {
            if($this->admin['group_id'] == 5){ // supplier
                $this->view('supplier_product_form', $data);
            }else {
        ...
        else
        {
            ...
            if(isset($_POST['confirm'])){
                $save['confirm'] = \CI::input()->post('confirm');
            }
            if($duplicate)
            {
                if($this->admin['group_id'] == 5){
	...
	function export_sitemap(){
		$file = fopen('sitemap.xml', 'w');
        $xml = $this->partial('sitemap_xml_head', [], true);
        fwrite($file, $xml);
        fclose($file);
		
        $products = \CI::Products()->products();
