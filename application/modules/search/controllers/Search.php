...
class Search extends Front {
    ...
    public function index($code=false, $sort='name', $dir="ASC", $page = 0)
    {
        ...
        if(!$code)
        {
            ...
            {
                $code = \CI::Search()->recordTerm(json_encode(\CI::input()->post()));
        ...
        else
        {
            ...
            $getTerm = \CI::Search()->getTerm($code);
			if($getTerm){
				$getTerm = json_decode($getTerm);
				$term = $getTerm->term;
				$cat_id = $getTerm->category_id;
				$cat_name = $getTerm->category_name;
			}
        }
        ...
        else
        {
            ...
            $result = \CI::Products()->search_products($term, $per_page, $page, $sort, $dir, $data['cat_id']);
