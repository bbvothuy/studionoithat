...
class GoCart {
    ...
    public function insertItem($data=[])
    {
        ...
        if(!$update && $product->product_id) // if not an update or non-product, try and run the options
        {
            if($product->saleprice > 0
                && ($product->sale_start_date == '' || date('Y-m-d') >= $product->sale_start_date)
                && ($product->sale_end_date == '' || date('Y-m-d') <= $product->sale_end_date)){

            }else{
                $product->saleprice = 0;
                $product->saleprice_1 = 0;
                $product->saleprice_2 = 0;
            }
    ...
    function submitOrder($transaction = false)
    {
        ...
        $this->cart->notes_customer = $_POST['notes_customer'];
		...
		// add invoice_number
		$invoice_number = CI::db()->order_by('invoice_number', 'DESC')->limit(1)->get('orders')->row();
        CI::db()->where('order_number', $orderNumber)->update('orders', array('invoice_number' => ($invoice_number->invoice_number + 1)));
    ...
    public function set_notes_customer($notes_customer = ''){
        //echo $notes_customer;
        $this->cart->notes_customer = $notes_customer;
        $order_id = $this->cart->id;
        \CI::db()->where('id', $order_id)->update('orders', array('notes_customer' => $notes_customer));
    }
}
