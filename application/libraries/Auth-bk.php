...
class Auth
{
	...
	public function check_access_group($module_name, $method = '', $redirect = false)
    {        
        $admin = CI::session()->userdata('admin');
		if($admin['group_id'] == 1)
			return true;
			...
			CI::db()->where('permissions.group_id', $admin['group_id']);
			CI::db()->where('permissions.active', 1);
			CI::db()->join('modules', 'modules.id=permissions.module_id');
			CI::db()->where('modules.module_name', $module_name);
			if($method != ''){
				CI::db()->where('modules.method', $method);
			}
			CI::db()->limit(1);
