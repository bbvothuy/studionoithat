...
class Bootstrap extends CI_Controller {
	...
	public function init()
	{
		...
		//get store
		$get_store = $this->Settings->get_stores($_SERVER['HTTP_HOST']);
		if(empty($get_store)){
			$this->config->set_item('store_name', 'default');
		}else{
			$this->config->set_item('store_name', $get_store->store_name);
			$this->config->set_item('store', $get_store);
		}
		//print_r($this->session->userdata('customer'));exit;
		//echo $this->db->last_query();print_r($store);exit;
		//print_r(config_item('store'));exit;
